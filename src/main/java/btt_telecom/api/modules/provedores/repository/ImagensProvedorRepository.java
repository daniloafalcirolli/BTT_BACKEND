...
public interface ImagensProvedorRepository extends JpaRepository<ImagemProvedor, Long>{
	...
	@Query(value = "SELECT * FROM IMAGENS_PROVEDOR WHERE UPPER(NAME) LIKE UPPER(%?1%) OR UPPER(DESCRICAO) LIKE UPPER(%?1%) OR UPPER(STATUS_SERVICO) LIKE UPPER(%?1%)", nativeQuery = true)
